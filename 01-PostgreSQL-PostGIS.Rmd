...
author <- yml_content[["postgresql-postgis"]][["author"]]
```

# PostgreSQL and PostGIS {#postgresql-postgis}
...
- Create and host your own PostgreSQL server
...
**Step 2:** Start pgAdmin 4. This software is a a management tool for PostgreSQL databases, hence "pg". 
...
```{r 01-pgadmin-new-server-connection, out.width= "50%", echo = FALSE}
...
```{r 01-pgadmin-connect, out.width= "75%", echo = FALSE}
...
```{r 01-pgadmin-ubcv-buildings, out.width= "75%", echo = FALSE}
...
```{r 01-pgadmin-ubcv-query, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/01-pgadmin-ubcv-query.png")
```

## Task 2: Explore the UBC PostgreSQL Database in QGIS {.unnumbered}

**Step 1:** Open QGIS. From the Browser pane on the left, right-click "PostgreSQL" and select "New Connection".

```{r 01-qgis-postgres-connection, out.width= "50%", echo = FALSE}
...
```{r 01-qgis-browser-ubcv, out.width= "50%", echo = FALSE}
...
```{r 01-qgis-query, out.width= "50%", echo = FALSE}
...
```{r 01-qgis-query-properties, out.width= "50%", echo = FALSE}
...
## Task 3: Explore the UBC PostgreSQL Database in ArcGIS Pro {.unnumbered}
...
```{r 01-arcgispro-postgres-connection, out.width= "50%", echo = FALSE}
...
```{r 01-arcgispro-postrges-catalog, out.width= "50%", echo = FALSE}
...
```{r 01-arcgispro-buildings-properties, out.width= "50%", echo = FALSE}
...
```{r 01-arcgispro-buildings-query, out.width= "50%", echo = FALSE}
...
## Task 4: Explore the UBC PostgreSQL Database via psql Shell  {.unnumbered}
...
```{r 01-psql-postgres-connection, out.width= "75%", echo = FALSE}
...
```{r 01-psql-list-tables, out.width= "75%", echo = FALSE}
...
```{r 01-psql-list-fields, out.width= "75%", echo = FALSE}
...
```{r 01-psql-count, out.width= "75%", echo = FALSE}
...
```{r 01-psql-distinct, out.width= "75%", echo = FALSE}
...
```{r 01-psql-select, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/01-psql-select.png")
```

## Task 5: Host your own PostgreSQL Server {.unnumbered}

**Step 1:** Return to or open pgAdmin.
...
**Step 4:** Open or return to the psql shell. 

The prompt should say `Server [localhost]:`. Press "Enter" on your keyboard to accept localhost as the server host name. 
...
If everything is successful, then you should see `postgres=#`, which indicates you can now enter either psql commands or SQL statements.

**Step 5:** Create a new database called "mydb" with the following SQL command `CREATE DATABASE mydb;`. It is important to always escape SQL statements with a semi-colon otherwise psql will interpret your input as spanning multiple lines! If you forget the semi-colon, you can always just type it in the console and hit "Enter" on your keyboard and psql will interpret this as a two-line statement.

**Step 6:** Connect to your new database by using the psql command `\c mydb`. Note that this is NOT an SQL statement, so there is no need to escape the command with a semi-colon. Your console should now say `mydb=#`, which indicates you are now connected. 

Some of useful psql commands:
