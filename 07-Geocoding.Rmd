...
**Step 2:** Create an address locator, which is located in the Geoprocessing pane by selecting "Analysis", "Tools", "Toolboxes", "Geocoding Tools", and "Create Locator". You can also use the search bar to search for the tool by name.
...
**Step 4:** Geocode a table of addresses using "GVRDaddresses". The table of addresses you will be geocoding is "neighborhood_trees" that you exported earlier. To access the geocoding tool, you can search for "Geocode Addresses".
...
Use the following tool parameters: 
...
**Step 5:** Look at the attribute table. The score column shows from 0-100 based on how close the address in the address locator was able to match the input table. The mailbox icon with the arrows in the geocoding toolbar allows you to investigate the matches more in-depth. With a messy dataset, this becomes invaluable.

##### Q3. Are all of the addresses a 100% match? Why or why not? (2 points) {.unnumbered}

**Step 6:** Toggle the "neighborhood_trees" and "geocoded" datasets on and off to look for similarities and differences.
...
**Step 2:** Right-click on the "parktrees" table in your Contents Pane and then select "Create Points From Table, then Make XY Event Layer". Choose the appropriate column names for the X and Y fields. Leave the Z field blank. This is very important because if you choose the wrong columns the points will be nowhere near Vancouver! Specify the coordinate system by clicking on the Globe icon to the right-hand side of Coordinate System and locating NAD 1983 UTM Zone 10N under Projected Coordinate System, UTM, and NAD 1983.
...
##### Q9. Which column did you choose for x, which column for y? (2 points) {.unnumbered}
 ...
 You have now created an "Event Layer." Export the Events layer as a shapefile (right-click the layer, select "Data", and then select "Export Features"). Save it in your output folder as "parktrees". Add "parktrees" to your display.
...
**Step 4:** Add "parks_polygon_representation" and a satellite basemap to your map (from the "Map" tab, select "Basemap", and then select "Imagery"). In the Contents Pane, drag the basemap to the bottom so that it does not cover anything. Symbolize "parks_polygon_representation" so that you can clearly see the basemap for each park. You can do this in the Contents Pane by right-clicking the coloured rectangle beneath "parks_polygon_representation" and choosing "Color Properties..." at the bottom. Adjust the colours and transparency as you see fit.
