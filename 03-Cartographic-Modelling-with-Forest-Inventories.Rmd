...
<input type="checkbox" unchecked> A screenshot of the python expression created for Task 2 (10 points)</input>
...
All data for this lab are accessible via the UBC PostgreSQL server. Instructions for connecting to the server are given in the tasks below. These are very large datasets that come from the BC Data Catalogue and are described in the table below. Accessing these data from the UBC PostgreSQL server allows us to extract only the data we need for this lab, which is the extent of Vancouver Island.

| Layer Name | File Name | Reference Link |
|------------------------|------------------------|------------------------|
| Vegetation Resource Inventory 2025 | `vancouver_island_vri` | <https://catalogue.data.gov.bc.ca/dataset/vri-2025-forest-vegetation-composite-layer-1-l1-> |
| Land Designations for Forest Management | `vancouver_island_own` | <https://catalogue.data.gov.bc.ca/dataset/4574f131-470e-407c-bbcc-5b19d9adcc1b> |
| Human Disturbance – 2025 | `vancouver_island_human_disturbance` | <https://catalogue.data.gov.bc.ca/dataset/human-disturbance-2025> |
| Landscape Units of British Columbia - Current | `vancouver_island_landscape_units` | <https://catalogue.data.gov.bc.ca/dataset/landscape-units-of-british-columbia-current> |

------------------------------------------------------------------------

## Task 1: Query and export data from UBC PostgreSQL server {.unnumbered}
...
```{r 03-arcgispro-postgres-connection-1, out.width= "100%", echo = FALSE}
    knitr::include_graphics("images/03-arcgispro-postgres-connection-1.png")
```

You can name it "UBC PostgreSQL Server" and for the host enter "FRST-GIS.ead.ubc.ca". The username and password will be provided to students enrolled in the course and it is not your CWL. Once you have correctly entered the correct credentials, ArcGIS Pro will do a "soft" connection to the server to retrieve the available databases. From the "Database" drop-down menu, select "vri" and then click "OK".

```{r 03-arcgispro-postgres-connection-2, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/03-arcgispro-postgres-connection-2.png")
```

The database connection has now been added to your ArcGIS Pro project, and you can view the tables inside the database from the Catalog Pane (open the "View" tab, open the "Catalog Pane", expand "Databases", expand "PostgreSQL-FRST-GIS-vri(student)", and view the tables in the `vri` database). You can drag and drop any of the layers of the database directly into your map to visualize them if you want. Though be aware that these are large layers and they may take a while to draw on your map. We do not actually need all of these data, so instead, we are going to use the power of SQL (Structured Query Language) to write a query that will return to us the tiny fraction that we need to use for this lab. First, let us break down what a generic SQL SELECT query looks like:

``` sql
SELECT feature_id, site_index, species_cd_1, species_pct_1, geom
...
What follows the `SELECT` keyword is a comma-separated list of column names that we want to retrieve from the table, `feature_id, site_index, species_cd_1, species_pct_1, geom`. You could also replace these with `*` to return all of the column names and this is generally the default behavior across ArcGIS Pro, so for most tasks in ArcGIS Pro you generally will not need to specify any column names when writing a SQL query.

-   `feature_id` is the primary key of the VRI table, this uniquely identifies all of the polygons from each other. When we select any subset of a table, we always want to grab the primary key because this allows us to keep track of which polygon we are dealing with from the originating table.

-   `site_index` is the estimate of site productivity for tree growth for the polygon; the values are recorded as decimal numbers.

-   `species_cd_1` is the leading tree species code for the polygon; the values are recorded as text strings.
...
There are 190 other attributes (columns) in the VRI table for each polygon, so this is just a small subset. You should review the [VRI Relational Data Dictionary](/data/standards/vegcomp_poly_rank1_data_dictionaryv5_2019.pdf) if you have any doubts about what a particular field refers to.

Next, the statement describes the table name in the database that these columns are being selected from, `FROM veg_comp_lyr_r1_poly_2025`.
...
**Step 6:** Using what you just learned, draft a select statement that limits the VRI polygons to only managed forests and return only the column names `feature_id`, `bec_zone_code`, `bec_subzone`, `proj_age_1`, and `geom`. Press the "Validate" button on the bottom of the dialogue window. If the statement validates successfully, then the "Next" button will allow you to proceed.
...
```{r 03-Export-Features, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/03-ExportFeatures.png")
```
...
```{r 03-select-by-attributes, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/03-select-by-attributes.png")
```

You can read more about the CEF human disturbance layer and methodology here: <https://catalogue.data.gov.bc.ca/dataset/bc-cumulative-effects-framework-human-disturbance-current>

**Step 11:** Next, we will select the areas where crown land overlaps with potentially forested areas.

Use the **Pairwise Intersect tool**. You can search for it by clicking the toolbox on the top ribbon (open the "Analysis" tab, then click "Tools").
...
```{r 03-select-symbology, out.width= "25%", echo = FALSE}
    knitr::include_graphics("images/03-select-symbology.png")
```

```{r 03-select-borderless-symbology, out.width= "25%", echo = FALSE}
    knitr::include_graphics("images/03-select-borderless-symbology.png")
```
...
```{r 03-calculate-field, out.width= "75%", echo = FALSE}
...
```{r 03-PythonSyntax, out.width= "100%", echo = FALSE}
    knitr::include_graphics("images/03-PythonSyntax.jpg")
```

Check that all the rows have been populated in the `Seral_stage` field.

##### Q2. Define "old growth" and cite at least one primary source. What characteristics might be overlooked by a definition of "old growth" that only accounts for stand age? (10 points) {.unnumbered}

**Step 15:** Our current layers only indicate where crown forests *could potentially occur*. In this step, we will reset the seral stage in areas that have experienced human disturbances such as roads, buildings, or logging. We are not removing these areas entirely, as they remain part of the landscape unit where old-growth forest could exist if it had not been cleared. We will use `vancouver_island_human_disturbance` to do so. Select by attribute where `cef_human_disturb_flag` is equal to `Human Disturb Current 20yr`. Export the selected attributes as a new feature named `Disturbed_areas`.

```{r 03-select-by-location, out.width= "100%", echo = FALSE}
    knitr::include_graphics("images/03-select-by-location.png")
```
...
-   **Input Features**: Crown_Forest

-   **Relationship**: Intersect

-   **Selecting Features**: Disturbed_areas

-   **Selection Type**: New Selection

Leave other options with their defaults. This may take a couple of minutes to complete.
...
-   Right-click the `Seral_Stage` field, then click "Calculate Field"
...
```{r 03-define-seral-stage, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/03-define-seral-stage.png")
```

**Pro tip:** Always clear your selection once you are done working with it. This prevents potential missteps in later steps.
...
``` python
str(!landscape_unit_id!) + "_" + str(!bec_zone_code!) + str(!bec_subzone!)
```

```{r 03-lu-bec-calculate-field, out.width= "100%", echo = FALSE}
    knitr::include_graphics("images/03-lu-bec-calculate-field.png")
```
...
-   Statistics Fields: `Shape_Area` as `SUM` and `bec_subzone` as `FIRST`
...
```{r 03-total-forest-area, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/03-total-forest-area.png")
```

**Step 3:** To identify areas of assumed old growth, we will calculate the area of only forests where `Seral_stage` equals `old`. We can select for old forest by using **Select by Attributes on CrownForest_LU** before using the **Dissolve** tool. Ensure that the "Use the selected records" option is switched on.

-   Select by Attribute: `Seral_Stage = "Old"`
-   Dissolve Fields: LU_BEC
-   Statistics Fields: `Shape_Area` as `SUM`
...
```{r 03-old-growth-area, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/03-old-growth-area.png")
```
...
```{r 03-VRI_FieldJoin, out.width= "50%", echo = FALSE}
...
**Step 6:** Classify Percent Compared to Provincial Targets.
...
-   Positive values exceed the high target,

-   Negative values fall short of the high target,
...
##### Screenshot 1. Upload a screenshot of your python expression (10 points) {.unnumbered}
...
```{r 03-diff-from-high, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/03-diff-from-high.png")
```
...
-   **North Arrow**: select "Insert", then select "North Arrow". Place it in a corner where it does not overlap important data.

-   **Legend**: select "Insert", then select "Legend". This should explain the symbology of your `DiffFromHigh` values.

-   **Scale Bar**: select "Insert", then select "Scale Bar". A useful addition if map distance matters.
...
When you are satisfied with the layout, go to **select "Share", then select "Export Layout"**. Choose **PNG** as the file format.

##### Q3. Describe the results of your final output. What is the distribution of old growth targets met across Vancouver Island? (10 points) {.unnumbered}

##### Q4. Select 3 other fields in the VRI layer and briefly discuss how they could provide further insight into current old growth status. (10 points) {.unnumbered}

##### Q5. How is VRI data collected and what limitations does it pose? (10 points) {.unnumbered}

##### Q6. How might the scale and resolution of GIS data affect the accuracy of old growth assessments? (10 points) {.unnumbered}

##### Table 1. A table with fields for total area, old-growth area, and percentage old growth. (10 points) {.unnumbered}

##### Map 1. A map showing the study area and the distribution of difference in calculated old-growth percentage and targets, by polygon. (20 points) {.unnumbered}
...
CBC. (2022, February 11). Fairy Creek protesters bid to have charges stayed. *CBC News*. <https://www.cbc.ca/news/canada/british-columbia/protest-court-proceedings-1.6348014>

Forest Analysis and Inventory Branch (2025). VRI - 2025 - Forest Vegetation Composite Rank 1 Layer (R1). British Columbia Data Catalogue.

LePage, P., & Banner, A. (2014). Long-term recovery of forest structure and composition after harvesting in the coastal temperate rainforests of northern British Columbia. *Forest Ecology and Management*, *318*, 250–260. [doi.org/10.1016/j.foreco.2014.01.031](https://doi.org/10.1016/j.foreco.2014.01.031)
